(function($) {
    ...
    $document.on({
        'ajax:beforeSend': function() {
            $.utils.clearFieldError('form-academy-edit');
            $(this).find('.btn').attr('disabled', 'disabled');
        },
        'ajax:success': function(e, data) {
            if (data.status) {
                ...
                if (data.code === 'invalid_data') {
                    for (var field in data.messages) {
                        $.utils.showFieldError('form-academy-edit', field, data.messages[field][0], 1);
                    }
                    $.utils.autoFocus('form-academy-edit', '.input-error');
                }
                else {
                }
            }
